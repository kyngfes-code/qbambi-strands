...
export async function storeItemImageEditor({ id, type, imageUrl }) {
  const supabase = await supabaseServer();

  if (!id || !type || !imageUrl) {
    return {
      error: "Missing id, type or image URL.",
    };
  }
  ...
  // Delete previous storage object (best effort)
  if (oldImageUrl) {
    try {
      const oldPath = oldImageUrl.split("/object/public/hair-image/")[1];
