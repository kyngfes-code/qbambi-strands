...
export async function POST(req) {
  ...
  if (id) {
    const { data: oldRow, error: fetchError } = await supabase
      .from("make-up-images") // storage bucket name
      .select("images")
      .eq("id", id)
      .single();
  ...
  // UPLOAD NEW FILE
  // ----------------------------------------
  const fileName = `makeup-${rowId}-${Date.now()}`;
  ...
  const { error: dbError } = await supabase
    .from("make-up-images") //table name
  ...
  if (oldImageUrl) {
    try {
      const oldPath = oldImageUrl.split("/make-up-images/")[1];
      if (oldPath) {
        await supabase.storage.from("make-up-images").remove([oldPath]);
      }
    } catch (err) {
